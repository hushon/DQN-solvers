...
def fullyconnected_layer_with_ReLU(_in, _out):
    W = tf.get_variable([3, 3, 3, 30], initializer = init_weight)
    b = tf.Variable
    return tf.nn.relu(tf.matmul(_in, W) + b)

def q_network():

def agent():

## First build two DQNs -- onlineDQN and targetDQN

## Build a simple replay memory
