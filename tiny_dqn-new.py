...
num_outputs_list = [64, 32] # number of units in input layer and hidden layer
...
def q_network(X_state, name):
    ...
    with tf.variable_scope(name) as scope:
        ...
        outputs = tf.contrib.layers.fully_connected(
            ...
            trainable=True,
            scope=None)
        future_obs = tf.contrib.layers.fully_connected(
            prev_layer,
            np.prod(env.observation_space.shape)*n_outputs,
            activation_fn=None,
            normalizer_fn=None,
            normalizer_params=None,
            weights_initializer=tf.contrib.layers.xavier_initializer(),
    ...
    return outputs,future_obs, trainable_vars_by_name

state_shape = (2*np.prod(env.observation_space.shape), )
X_state = tf.placeholder(tf.float32, shape=[None]+list(state_shape))
online_q_values, online_future_obs, online_vars = q_network(X_state, name="q_networks/online")
target_q_values, target_future_obs, target_vars = q_network(X_state, name="q_networks/target")
...
with tf.variable_scope("train"):
    ...
    y_obs = tf.placeholder(tf.float32, shape=[None]+list(env.observation_space.shape))
    print (tf.shape(y_obs))
    print (tf.shape(online_future_obs))
    print (tf.shape(online_future_obs[X_action*np.prod(env.observation_space.shape):(X_action+1)*np.prod(env.observation_space.shape)]))
    error_obs = tf.abs(y_obs - online_future_obs[X_action*np.prod(env.observation_space.shape):(X_action+1)*np.prod(env.observation_space.shape)])
    loss_obs = tf.reduce_mean(tf.square(error_obs))

    loss = tf.reduce_mean(tf.square(clipped_error) + linear_error + loss_obs)
...
with tf.Session() as sess:
    ...
    while True:
        ...
        future_obs = online_future_obs.eval(feed_dict={X_state: [state]})[action*np.prod(env.observation_space.shape):(action+1)*np.prod(env.observation_space.shape)]
        intrinsic_reward = np.mean(np.abs(obs-future_obs))
        ...
        y_val = rewards + continues * discount_rate * max_next_q_values + intrinsic_reward

        # Train the online DQN
        _, loss_val = sess.run([training_op, loss], feed_dict={
            X_state: X_state_val, X_action: X_action_val, y: y_val, y_obs: obs})
